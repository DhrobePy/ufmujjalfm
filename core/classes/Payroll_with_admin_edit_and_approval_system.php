...
class Payroll {
    ...
    public function update_single_payroll_record($payroll_id, $data) {
        // Recalculate totals based on admin's adjustments
        $total_deductions = ($data['absence_deduction'] ?? 0) + 
                              ($data['advance_deduction'] ?? 0) + 
                              ($data['loan_deduction'] ?? 0) +
                              ($data['standard_deductions'] ?? 0); // Assuming other standard deductions might be added
                              
        $net_salary = $data['gross_salary'] - $total_deductions;
    
        $sql = "UPDATE payrolls SET 
                    gross_salary = :gross_salary, 
                    deductions = :deductions, 
                    net_salary = :net_salary 
                WHERE id = :payroll_id AND status = 'pending_approval'";
