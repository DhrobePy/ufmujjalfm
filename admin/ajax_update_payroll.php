...
$data = [
    ...
    'other_loan_repayment' => (float)($_POST['other_loan_repayment'] ?? 0), // New field
    'random_loan_id' => (int)($_POST['random_loan_id'] ?? 0) // The ID of the random loan being paid
];
...
if ($payroll_handler->update_single_payroll_record($payroll_id, $data)) {
    $net_salary = $data['gross_salary'] - ($data['absence_deduction'] + $data['advance_deduction'] + $data['loan_deduction'] + $data['other_loan_repayment']);
    echo json_encode([
        'success' => true, 
        ...
        'newData' => [
            ...
            'other_loan_repayment' => number_format($data['other_loan_repayment'], 2),
