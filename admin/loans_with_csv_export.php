...
<div class="space-y-6">
    ...
    <a href="export_all_loans.php" class="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 flex items-center gap-2 w-full sm:w-auto justify-center">
        <i class="fas fa-download"></i>
        Export Master Report (All Types)
    </a>
</div>
                ...
                <div class="overflow-x-auto">
                    ...
                    <tbody class="divide-y">
                        <?php foreach ($allLoans as $loan): ?>
                            <tr>
                                <td class="px-6 py-4"><?php echo htmlspecialchars($loan->first_name . ' ' . $loan->last_name); ?></td>
                                <td class="px-6 py-4"><?php echo date('d M, Y', strtotime($loan->loan_date)); ?></td>
                                <td class="px-6 py-4 text-right">৳<?php echo number_format($loan->amount, 2); ?></td>
                                <td class="px-6 py-4 text-right text-red-600 font-semibold">৳<?php echo number_format($loan->outstanding_balance, 2); ?></td>
                                <td class="px-6 py-4 text-center"><?php echo ucfirst($loan->status); ?></td>
                                <td class="px-6 py-4 text-center">
                                    <?php if ($loan->outstanding_balance > 0 && $loan->status === 'active'): ?>
                                    <button @click="paymentModal = true; selectedLoan = { id: 
                                        <?php echo $loan->id; ?>, amount: <?php echo $loan->amount; ?>, outstanding: <?php echo $loan->outstanding_balance; ?>, date: '<?php echo date('d M, Y', strtotime($loan->loan_date)); ?>', employee: '<?php echo htmlspecialchars($loan->first_name . ' ' . $loan->last_name); ?>' }; paymentAmount = ''; redirectTab = 'all_loans';" class="px-3 py-1 text-sm bg-blue-500 text-white rounded-md hover:bg-blue-600">Add Payment</button>
                                    <?php else: ?>
