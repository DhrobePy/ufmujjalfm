...
$bot_token = '8406596906:AAHRlrgmiaXqfbdOUvzP_-2f6km95WSpO7I'; // <--- PASTE YOUR TOKEN HERE
$chat_id   = '-5051698271'; // The Group ID you found
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['apply_for_loan'])) {
        ...
        // --- START: NEW TELEGRAM NOTIFICATION LOGIC ---
        if ($send_notifications && $bot_token !== 'YOUR_BOT_TOKEN_FROM_BOTFATHER') {
            try {
                // Get employee name
                $emp = $db->query("SELECT first_name, last_name FROM employees WHERE id = ?", [$employee_id])->first();
                $employee_name = $emp ? ($emp->first_name . ' ' . $emp->last_name) : 'Unknown';

                // Get who applied
                $applied_by = ($currentUser['id'] == $employee_id) ? "themselves" : $currentUser['full_name'];

                // Format loan type
                $loan_type_text = 'Unknown';
                if ($loan_type === 'fixed_emi') $loan_type_text = 'Fixed EMI Loan';
                if ($loan_type === 'random_repayment') $loan_type_text = 'Random Repayment Loan';
                if ($loan_type === 'salary_advance') $loan_type_text = 'Salary Advance';

                // Build the message
                $message = "🔔 *New Loan Application*\n\n";
                $message .= "A new loan application has been submitted and is awaiting approval.\n\n";
                $message .= "*Applicant:* " . $employee_name . "\n";
                $message .= "*Type:* " . $loan_type_text . "\n";
                $message .= "*Amount:* *৳" . number_format($amount, 2) . "*\n";
                
                if ($loan_type === 'fixed_emi' && !empty($_POST['installments'])) {
                    $message .= "*Installments:* " . (int)$_POST['installments'] . " months\n";
                }
                if ($loan_type === 'salary_advance') {
                    $month = $_POST['advance_month'];
                    $year = $_POST['advance_year'];
                    $period = date('F Y', mktime(0, 0, 0, $month, 1, $year));
                    $message .= "*For Month:* " . $period . "\n";
                }
                
                $message .= "*Reason:* " . $_POST['reason'] . "\n\n";
                $message .= "_Submitted by: " . $applied_by . "_";
                ...
                $ch = curl_init();
                // Set cURL options silently (errors will not stop the page)
                ...
                @curl_exec($ch); // We send the message but don't care about the response
                @curl_close($ch);

            } catch (Exception $e) {
                // Do nothing if Telegram fails; the main application was saved.
            }
        }
    ...
    if (isset($_POST['update_loan_application']) && $isAdmin) {
        ...
        $action = $_POST['action'];
        $app = $db->query("SELECT la.*, e.first_name, e.last_name FROM loan_applications la JOIN employees e ON la.employee_id = e.id WHERE la.id = ?", [$application_id])->first();
        ...
        if ($app) {
            if ($action === 'approve') {
                ...
                try {
                    ...
                    // --- START: TELEGRAM NOTIFICATION (APPROVAL) ---
                    if ($send_notifications && $bot_token !== 'YOUR_BOT_TOKEN_FROM_BOTFATHER') {
                        try {
                            $message = "✅ *Loan Application Approved*\n\n";
                            $message .= "*Applicant:* " . $app->first_name . ' ' . $app->last_name . "\n";
                            $message .= "*Amount:* *৳" . number_format($app->amount, 2) . "*\n";
                            $message .= "*Approved by:* " . $currentUser['full_name'];
                            ...
                            @curl_setopt($ch, CURLOPT_POSTFIELDS, $payload);
                            @curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                            @curl_setopt($ch, CURLOPT_TIMEOUT, 5); @curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 5);
                            @curl_exec($ch);
                            @curl_close($ch);
                        } catch (Exception $e) {}
                    }
                    // --- END: TELEGRAM NOTIFICATION (APPROVAL) ---
                ...
                // --- START: TELEGRAM NOTIFICATION (REJECTION) ---
                if ($send_notifications && $bot_token !== 'YOUR_BOT_TOKEN_FROM_BOTFATHER') {
                    try {
                        $message = "❌ *Loan Application Rejected*\n\n";
                        $message .= "*Applicant:* " . $app->first_name . ' ' . $app->last_name . "\n";
                        $message .= "*Amount:* ৳" . number_format($app->amount, 2) . "\n";
                        $message .= "*Rejected by:* " . $currentUser['full_name'];
                ...
                // --- END: TELEGRAM NOTIFICATION (REJECTION) ---
            }
        }
    ...
    // --- START: NEW DELETE LOAN LOGIC ---
    if (isset($_POST['delete_loan'])) {
        // CRITICAL: Only allow superadmin to perform this action
        if ($currentUser['role'] !== 'superadmin') {
            $_SESSION['error_flash'] = 'You do not have permission to perform this action.';
            header('Location: loans.php?tab=all_loans');
            exit();
        }

        $loan_id = (int)$_POST['loan_id'];

        // Use a transaction to delete the loan and all its installments
        $db->getPdo()->beginTransaction();
        try {
            // 1. Delete all related installment payments first
            $db->query("DELETE FROM loan_installments WHERE loan_id = ?", [$loan_id]);
            
            // 2. Delete the main loan record
            $db->query("DELETE FROM loans WHERE id = ?", [$loan_id]);
            
            // 3. Commit the changes
            $db->getPdo()->commit();
            $_SESSION['success_flash'] = 'The loan and all its associated payments have been permanently deleted.';

        } catch (Exception $e) {
            // If anything fails, roll back
            $db->getPdo()->rollBack();
            $_SESSION['error_flash'] = 'Error deleting loan: ' . $e->getMessage();
        }

        header('Location: loans.php?tab=all_loans');
        exit();
    }
    // --- END: NEW DELETE LOAN LOGIC ---
}
...
<div class="space-y-6">
    <div class="bg-white rounded-2xl shadow-xl border p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-3">
            <div class="h-12 w-12 bg-primary-100 rounded-xl flex items-center justify-center">
                <i class="fas fa-landmark text-primary-600 text-xl"></i>
            </div>
            Loan Management
        </h1>
        ...
        <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            ...
            <a href="export_all_loans.php" class="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 flex items-center gap-2 justify-center">
                <i class="fas fa-download"></i>
                Download Report
            </a>
            
            <a href="send_telegram_report.php" 
               class="px-4 py-2 text-sm bg-sky-500 text-white rounded-lg font-medium hover:bg-sky-600 flex items-center gap-2 justify-center"
               onclick="this.innerHTML = '<i class=\'fas fa-spinner fa-spin\'></i> Sending...'; this.classList.add('opacity-50', 'cursor-not-allowed');">
            ...
            <div x-show="activeTab === 'all_loans'" x-cloak><?php if ($isAdmin): ?>
                <div class="bg-white rounded-2xl shadow-xl border">
                    <div class="p-6 border-b flex justify-between items-center">
                        <h2 class="text-xl font-bold">All Employee Loans</h2>
                        <a href="export_loans.php" class="px-4 py-2 text-sm bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 flex items-center gap-2">
                    ...
                    <div class="overflow-x-auto"><table class="min-w-full divide-y">
                        ...
                        <tbody class="divide-y"><?php foreach ($allLoans as $loan): ?><tr><td class="px-6 py-4"><?php echo htmlspecialchars($loan->first_name . ' ' . $loan->last_name); ?></td><td class="px-6 py-4"><?php echo date('d M, Y', strtotime($loan->loan_date)); ?></td><td class="px-6 py-4 text-right">৳<?php echo number_format($loan->amount, 2); ?></td><td class="px-6 py-4 text-right text-red-600 font-semibold">৳<?php echo number_format($loan->outstanding_balance, 2); ?></td><td class="px-6 py-4 text-center"><?php echo ucfirst($loan->status); ?></td>
                        
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                
                                <?php if ($loan->outstanding_balance > 0 && $loan->status === 'active'): ?>
                                    <button @click="paymentModal = true; selectedLoan = { id: <?php echo $loan->id; ?>, amount: <?php echo $loan->amount; ?>, outstanding: <?php echo $loan->outstanding_balance; ?>, date: '<?php echo date('d M, Y', strtotime($loan->loan_date)); ?>', employee: '<?php echo htmlspecialchars($loan->first_name . ' ' . $loan->last_name); ?>' }; paymentAmount = ''; redirectTab = 'all_loans';" class="px-3 py-1 text-sm bg-blue-500 text-white rounded-md hover:bg-blue-600">
                                        Add Payment
                                    </button>
                                <?php else: ?>
                                    <span class="px-3 py-1 text-sm text-green-600 font-semibold">Paid</span>
                                <?php endif; ?>
                        
                                <?php if ($currentUser['role'] === 'superadmin'): ?>
                                    <form method="POST" class="inline-block" onsubmit="return confirm('WARNING: Are you sure you want to permanently delete this loan and all its payments? This action cannot be undone.');">
                                        <input type="hidden" name="loan_id" value="<?php echo $loan->id; ?>">
                                        <button type="submit" name="delete_loan" class="px-3 py-1 text-sm bg-red-600 text-white rounded-md hover:bg-red-700">
