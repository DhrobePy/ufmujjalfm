<?php
// new_ufmhrm/admin/attendance.php (Fully Corrected and Complete)
    ...
    if (isset($_POST['bulk_historical_update'])) {
        ...
        if (!empty($employee_ids) && !empty($historical_date)) {
            ...
            try {
                foreach ($employee_ids as $employee_id) {
                    $employee_id = (int)$employee_id;
                    $existing = $db->query("SELECT id FROM attendance WHERE employee_id = ? AND DATE(clock_in) = ?", [$employee_id, $historical_date])->first();
                    ...
                    if ($existing) {
                        ...
                        $db->insert('attendance', ['employee_id' => $employee_id, 'clock_in' => $clock_in, 'clock_out' => $clock_out, 'status' => $status, 'manual_entry' => 1]);
                    }
                }
                $db->getPdo()->commit();
                $_SESSION['success_flash'] = count($employee_ids) . ' records updated for ' . date('F j, Y', strtotime($historical_date));
            } catch (Exception $e) {
                $db->getPdo()->rollBack();
                $_SESSION['error_flash'] = 'An error occurred: ' . $e->getMessage();
            }
        }
        header('Location: attendance.php?tab=bulk_entry&date=' . $historical_date);
        exit();
    }
}
...
$sheetSql = "SELECT e.id, e.first_name, e.last_name, p.name as position_name, d.name as department_name, a.status, a.clock_in, a.clock_out FROM employees e LEFT JOIN positions p ON e.position_id = p.id LEFT JOIN departments d ON p.department_id = d.id LEFT JOIN attendance a ON e.id = a.employee_id AND DATE(a.clock_in) = ? WHERE e.status = 'active' ORDER BY e.first_name, e.last_name";
...
$manualEntrySql = "SELECT e.id, e.first_name, e.last_name, p.name as position_name, d.name as department_name FROM employees e LEFT JOIN positions p ON e.position_id = p.id LEFT JOIN departments d ON p.department_id = d.id WHERE e.status = 'active' AND e.id NOT IN (SELECT employee_id FROM attendance WHERE DATE(clock_in) = CURDATE() AND status = 'present') ORDER BY e.first_name, e.last_name";
$manualEntryList = $db->query($manualEntrySql)->results();

// --- DATA FOR TAB 3: BULK HISTORICAL ENTRY ---
// CORRECTED QUERY: Specifically select e.id to avoid ambiguity
$allEmployeesSql = "SELECT e.id, e.first_name, e.last_name, d.name as department_name FROM employees e LEFT JOIN positions p ON e.position_id = p.id LEFT JOIN departments d ON p.department_id = d.id WHERE e.status='active' ORDER BY e.first_name";
$allEmployees = $db->query($allEmployeesSql)->results();
...
$historySql = "SELECT a.clock_in, a.clock_out, a.status, e.first_name, e.last_name, d.name as department_name FROM attendance a JOIN employees e ON a.employee_id = e.id LEFT JOIN positions p ON e.position_id = p.id LEFT JOIN departments d ON p.department_id = d.id $whereSql ORDER BY a.clock_in DESC";
...
<div class="space-y-6">
    ...
    <div x-data="{ activeTab: 'sheet' }" x-init="()=>{ const params = new URLSearchParams(window.location.search); if (params.get('tab')) { activeTab = params.get('tab'); } window.history.replaceState({}, document.title, window.location.pathname); }">
        <div class="border-b border-gray-200"><nav class="-mb-px flex space-x-8" aria-label="Tabs">
            ...
            <a href="#biometric" @click.prevent="activeTab = 'biometric'" :class="{ 'border-primary-500 text-primary-600': activeTab === 'biometric' }" class="border-transparent text-gray-500 hover:border-gray-300 py-4 px-1 border-b-2 font-medium text-sm flex items-center gap-2"><i class="fas fa-fingerprint"></i> Biometric</a>
        </nav></div>
        ...
        <div class="mt-6">
            ...
            <div x-show="activeTab === 'bulk_entry'" x-cloak>
                 <div class="bg-white rounded-2xl shadow-xl border">
        <div class="p-6 border-b">
            <h2 class="text-xl font-bold">Bulk Historical Attendance Entry</h2>
